...
class ID3DTree(object):
    ...
    def loadDataSet(self, path, labels):
        with open(path) as fp:
            content = fp.read()
        self.dataSet = [row.split("\t") for row in content.splitlines()]
        self.labels = labels
    ...
    def buildTree(self, dataSet, labels):
        ...
        # 程序终止条件1: 如果classList只有一种决策标签，只有yes或no，则停止划分，返回这个决策标签
        ...
        for value in uniqueVals:
            ...
            splitDataset = self.splitDataSet(
                dataSet, bestFeat, value)  # 按最优特征列和值分割数据集
        ...
        for i in range(numFeatures):		# 抽取第i列的列向量
            ...
            for value in uniqueVals:         # 内循环：按列和唯一值计算香农熵
                subDataSet = self.splitDataSet(
                    dataSet, i, value)  # 按选定列i和唯一值分隔数据集
    ...
    def computeEntropy(self, dataSet):                         # 计算香农熵
        ...
        items = dict([(i, cateList.count(i))
                      for i in cateList])  # 得到类别为key，出现次数value的字典
